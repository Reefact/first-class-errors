...
public sealed class OutcomeTaskExtensionsEagerValidationTests {
    ...
    public void RecoverOverANullGenericTaskOutcomeThrowsSynchronously() {
        // Exercise & verify
        Check.ThatCode(() => { _ = ((Task<Outcome<int>>)null!).Recover(_ => Outcome<int>.Success(JustDummies.Any.Int32().Generate())); })
             .Throws<ArgumentNullException>();
    }
    ...
    public void ToWithANullConversionThrowsSynchronously() {
        // Setup
        Task<Outcome<int>> task = Task.FromResult(Outcome<int>.Success(JustDummies.Any.Int32().Generate()));
