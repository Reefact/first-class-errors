...
public static class InstanceIds {
    ...
    ///     collide), drawn from JustDummies' ambient random context rather than <see cref="System.Guid.NewGuid" />. Run the
    ///     test inside <c>JustDummies.Any.Reproducibly(...)</c> to make the sequence reproducible and reported on failure. To
    ///     pin a single fixed id instead, use <see cref="UseFixed" />.
    /// </remarks>
    /// <returns>A scope that restores the default (random) identifier when disposed.</returns>
    public static IDisposable UseAny() {
        return Use(() => JustDummies.Any.Guid().Generate());
    }
