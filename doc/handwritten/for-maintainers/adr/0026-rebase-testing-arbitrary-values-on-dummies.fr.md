# ADR-0026 | Rebaser les valeurs de test arbitraires sur JustDummies
...
Depuis l'ADR-0006, cet utilitaire a été livré. L'ADR-0011 a introduit `JustDummies` :
...
* **Deux moteurs indépendants coexistent.** `FirstClassErrors.Testing` puise dans
  sa propre source à graine ; `JustDummies` puise dans la sienne. Chacun expose sa
...
* **Les deux façades `Any` entrent en collision de noms de type.**
  `FirstClassErrors.Testing.Any` et `JustDummies.Any` sont toutes deux
  `static class Any` ; un fichier de test important les deux espaces de noms ne
  peut pas nommer `Any` sans qualification.
* **`JustDummies` couvre déjà toutes les capacités dont le rebase a besoin.** Son
...
* **Le vocabulaire d'erreur ne peut pas migrer dans `JustDummies`.** Les utilitaires
  ...
  frontière de l'ADR-0011 interdit à `JustDummies` de référencer.
* **`JustDummies` n'est pas encore sur NuGet.** L'ADR-0011 ne lui donne aucun train de
  release avant sa première publication ; au sein de ce dépôt, il n'est consommé
  qu'au travers d'une référence de projet.

## Décision

`FirstClassErrors.Testing` puise toute valeur arbitraire dans `JustDummies` plutôt que
...
ambiant reproductible de JustDummies, et le vocabulaire d'erreur qu'il conserve est
...
* **Un moteur, une histoire de graine — l'esprit « source unique » de l'ADR-0006 à
  ...
  dans le contexte ambiant de JustDummies place les primitives, les valeurs métier,
...
* **Elle réalise l'action de suivi anticipée par l'ADR-0006 *et* l'ADR-0011.** Le
  ...
  sous le nom de `JustDummies`, et le « second consommateur » qu'il attendait est
  arrivé — les projets de tests de ce dépôt. Réasseoir `Testing` sur `JustDummies`
...
* **Le rebase n'ajoute aucun manque de capacité.** Tout comportement dont le
  package a besoin existe déjà dans JustDummies : l'exclusion de membres pour les
  ...
  être ajouté à JustDummies comme préalable, si bien que le package est façonné par un
...
* **Le vocabulaire reste dans `Testing`, mais en fabriques, pas en façade.** Parce
  ...
  `JustDummies`, il demeure dans `Testing` ; le réexprimer en types `…Factory` nommés
  plutôt qu'en un second `Any` supprime la collision de noms
  `Testing.Any`/`JustDummies.Any`, laissant `JustDummies.Any` comme l'unique `Any` qu'un
...
* **C'est le moment le moins cher.** `Testing` est en `0.1.0-preview.1`, un package
  ...
  JustDummies s'applique ici.

## Alternatives envisagées

### Garder le moteur propre à `Testing` et laisser les projets de tests utiliser `JustDummies` directement

Envisagé parce que c'est le moindre travail et que cela livre aujourd'hui : rien
ne change dans `Testing`, et les projets de tests ajoutent simplement `JustDummies`
...
composent pas et une collision de noms `Testing.Any`/`JustDummies.Any` — soit la
...
### Déplacer le vocabulaire d'erreur dans `JustDummies`

Envisagé parce qu'il ne resterait qu'un seul `Any` et un seul foyer pour les
valeurs arbitraires. Rejeté parce que `JustDummies` est agnostique aux erreurs par
...
JustDummies. Rejeté parce que cela taxe le cas dominant « donne-moi une valeur
...
* `JustDummies.Any` est l'unique `Any` ; la collision de noms de type disparaît, et les
...
* `Testing` cesse de maintenir un moteur de valeurs parallèle ; la machinerie
  générique vit une seule fois, dans `JustDummies`, façonnée par un consommateur réel
  du dépôt.

### Négatives

* Un changement cassant sur un package publié : `Testing.Any` et son `Reproducibly`
  sont retirés, et les consommateurs migrent vers `JustDummies.Any` et les fabriques.
  Acceptable en `0.1.0-preview.1`, et la raison pour laquelle la décision est prise
  avant une release stable.
* `Testing` gagne une dépendance sur `JustDummies`. Tant que `JustDummies` n'est pas sur
  NuGet, le package `Testing` doit embarquer `JustDummies` dans son propre artefact
  pour rester restaurable — un arrangement d'intérim, non l'état final.
* La suite de tests migre de `Any.*` vers `JustDummies.Any.*().Generate()` et les
...
* Embarquer `JustDummies` dans l'artefact `Testing` devient un risque d'identité de
  type dès l'instant où `JustDummies` est référençable indépendamment sur NuGet — deux
  assemblies `JustDummies` de même identité mais d'origine distincte — précisément
  parce que des types JustDummies apparaissent dans l'API publique de `Testing`.
  Atténué par l'action de suivi consistant à basculer vers une dépendance NuGet dès
  la première publication de JustDummies.
...
* À l'acceptation, l'ADR-0006 est supersédé par le présent ADR (son statut passe à
  *Superseded* avec un lien vers ici), et l'action de suivi ouverte de l'ADR-0011 —
  décider si `Testing` se réasseoit sur `JustDummies` — est résolue par cette décision.
* Basculer la dépendance `JustDummies` que `Testing` embarque dans son package vers un
  `PackageReference` NuGet dès la première publication de `JustDummies`, dénouant
...
* Mettre à jour le guide de test destiné aux utilisateurs et le README du package
  ...
  les valeurs arbitraires proviennent de `JustDummies`, et les fabriques métier sont
  introduites.
* Ajouter un petit projet `FirstClassErrors.Testing.UnitTests` ne portant que les
  ...
  valeur enveloppante désormais couvertes en transitif et par `JustDummies.UnitTests`.
...
* ADR-0011 — Héberger JustDummies comme package autonome : la frontière agnostique aux
