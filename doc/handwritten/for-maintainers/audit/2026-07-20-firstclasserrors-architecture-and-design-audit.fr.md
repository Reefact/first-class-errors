...
17. [Suivi des issues](#17-suivi-des-issues)
...
4. **Une dérive de périmètre délibérée et gouvernée : JustDummies.** Une bibliothèque générique de données de test, dont l'audience dépasse explicitement ce projet, vit dans le dépôt de la bibliothèque d'erreurs. Point crucial, la dérive est *bornée par ADR* (l'ADR-0011 consigne la décision, ses coûts et ses déclencheurs d'extraction), appliquée par machine (un test d'architecture plus un garde-fou nuspec au moment du pack prouvent que JustDummies ne référence aucun assemblage FirstClassErrors), et honnête. Elle n'en dilue pas moins l'identité mono-produit d'un dépôt pré-1.0 ; les §10 et §15 y reviennent.
...
| 0011 | JustDummies comme paquet autonome dans le dépôt | Solide | Conforme | Actuelle |
...
| 0026 | Rebaser les valeurs arbitraires de Testing sur JustDummies | Solide | **Partielle** | Actuelle |
...
1. **La prémisse de risque de l'ADR-0026 est imprécise et un suivi a été manqué (la seule lacune de conformité du corpus).** Le risque principal consigné dit que le danger d'assemblage embarqué survient « precisely because JustDummies types appear in Testing's public API » — or aucun membre public de `FirstClassErrors.Testing` n'expose aujourd'hui un type JustDummies ; le vrai danger est que des consommateurs compilent contre la `JustDummies.dll` embarquée dans `lib/` (le README enjoint `JustDummies.Any.*`). Même danger, mécanisme différent — à consigner correctement dans la référence d'implémentation. Plus concrètement : le suivi de lockstep de la doc a été manqué sur `DeterministicTesting.en.md`/`.fr.md` (ligne ~163), qui décrit toujours l'API de graine *remplacée* (`UseAny()` ne prend pas de graine ; la reproductibilité vient désormais de `JustDummies.Any.Reproducibly(...)`) — une contradiction, visible de l'utilisateur, avec l'API livrée, que la règle de synchronisation inscrite au CLAUDE.md du projet rend obligatoire de corriger. Enfin, la Décision promet des méthodes de fabrique exposant des générateurs `IAny<T>` « where composition is needed », et aucune n'existe encore — dormance voulue, mais la Décision se lit comme une forme livrée.
...
Côté code, l'ADR-0026 est pleinement réalisée : le moteur privé du paquet Testing et sa façade `Any` ont disparu ; les fabriques tirent de JustDummies (`ErrorCodeFactory` utilise `JustDummies.Any.StringMatching("ANY_CODE_[A-Z0-9]{6}").As(ErrorCode.Create).Generate()`) ; les points de couture d'horloge et d'identifiant d'instance participent au contexte reproductible ambiant de JustDummies, si bien qu'**une seule graine `Reproducibly` rejoue un run de test entier** — le bénéfice annoncé de la décision.
...
3. **Un risque consigné imprécis :** le danger de la `JustDummies.dll` embarquée est réel mais passe par des consommateurs qui compilent contre l'assemblage embarqué, pas (comme consigné) par des types JustDummies dans l'API publique de Testing — à corriger dans la référence d'implémentation, puisque les ADR acceptées sont immuables.
...
| `JustDummies` | netstandard2.0; net8.0 | `dum` (tag `dum-v*`) | Bibliothèque autonome de données de test (ADR-0011) |
...
**État sur nuget.org (vérifié pendant l'audit) :** `FirstClassErrors` et `FirstClassErrors.Testing` sont publiés en `0.1.0-preview.1` ; `FirstClassErrors.RequestBinder`, `FirstClassErrors.Cli` et `JustDummies` sont des identifiants non réclamés.
...
* La règle d'autonomie de JustDummies (ADR-0011) est contrôlée au moment du build par un test d'architecture (`JustDummies.UnitTests/ArchitectureTests.cs`) *et* de nouveau sur l'artefact livré par une assertion nuspec au moment du pack (`tools/packaging/pack.sh`).
...
**(1) `FirstClassErrors.Testing` fait passer en contrebande une `JustDummies.dll` non publiée dans son propre dossier `lib/` (Élevé).** Parce que JustDummies n'est pas encore sur NuGet, Testing la référence avec `PrivateAssets=all` et embarque la DLL via une cible de pack personnalisée (`IncludeJustDummiesInPackage`). Quatre conséquences : la version de JustDummies qu'exécute un consommateur est invisible pour NuGet et ingérable par lui ; une fois JustDummies publiée, un consommateur référençant à la fois Testing et JustDummies obtient deux assemblages de même identité sans aucun mécanisme de résolution ; comme Testing ne cible que netstandard2.0, la copie embarquée est l'asset JustDummies *downlevel*, si bien que les consommateurs net8+ perdent silencieusement les générateurs de types modernes (`DateOnly`/`TimeOnly`/`Int128`/`Half`) que la cible net8.0 du vrai paquet leur donnerait ; et conformément à l'ADR-0026, les types JustDummies (`IAny<T>`) font partie de l'*API publique* de Testing, si bien qu'il s'agit d'une dépendance cachée envers un fournisseur de types de surface publique. Le csproj documente le plan de sortie (« switch to a PackageReference once JustDummies is published »), ce qui atténue le risque sans le supprimer tant que la fenêtre est ouverte. C'est le défaut le plus lourd de conséquences de l'écosystème, et il se résout en exécutant une décision déjà prise.

**(2) L'identifiant de paquet générique `JustDummies` est non réclamé alors que son train de release existe déjà (Moyen).** La prémisse même de l'ADR-0011 est qu'« une identité de paquet est coûteuse à renommer après adoption ». L'identifiant est un mot anglais courant, vérifié non réclamé au moment de l'audit ; jusqu'au premier push, n'importe quel tiers peut le prendre, et la réservation de préfixe NuGet ne peut pas protéger un mot générique isolé de la même façon que `FirstClassErrors.*` est de facto ancré par ses deux paquets publiés. Chaque garde-fou nécessaire à une première publication sûre existe déjà dans `pack.sh`. C'est un pur risque d'exécution, qui se solde à peu de frais.
...
**Proposition.** Deux couches, suivant le précédent des paquets satellites propre au dépôt (RequestBinder, Testing, JustDummies) :
...
| C2 | **Refermer la fenêtre de séquencement de publication :** publier (ou a minima réserver) l'identifiant `JustDummies` ; à la première release lib qui suit, remplacer la `JustDummies.dll` embarquée dans Testing par une vraie `PackageReference` | Les deux risques ne sont ouverts *que* jusqu'à la première publication ; chaque garde-fou nécessaire existe déjà | §10.3 |
...
17. **Combler le manque d'adoption de la reproductibilité :** construire l'adaptateur xUnit façon `[ReproducibleFact]` (ou envelopper dans `Reproducibly` les suites riches en valeurs arbitraires) ; donner à JustDummies une section d'accueil dans le README et un guide utilisateur avant `dum-v0.1.0` (§12.5, §10.3).
...
* **C2 : publier JustDummies** (pour revendiquer l'identifiant), puis remplacer la DLL embarquée par une `PackageReference` au prochain pack de la bibliothèque ; doter JustDummies de sa documentation d'accueil.
...
Les recommandations du §14 ont été ouvertes comme issues GitHub le 2026-07-20, selon le format établi par l'audit JustDummies (#206–#226). Cette table est un **instantané statique** : l'état vivant de chaque issue (ouverte, fermée, en cours) vit dans le tracker, pas ici — ne pas maintenir de statut dans ce document. Les rejets hors périmètre (§13.6) ne sont volontairement pas suivis : l'audit recommande de les maintenir rejetés.

| Élément du §14 | Issue(s) | Phase (§15) |
|---|---|---|
| C1 — Rédiger a posteriori les ADR fondatrices | [#228](https://github.com/Reefact/first-class-errors/issues/228) | 1 |
| C2 — Publier JustDummies ; retirer la DLL embarquée | [#229](https://github.com/Reefact/first-class-errors/issues/229) | 1 |
| C3 — Corriger les vitrines (README) | [#230](https://github.com/Reefact/first-class-errors/issues/230) | 0 |
| C4 — Corriger les quatre dérives confirmées | [#231](https://github.com/Reefact/first-class-errors/issues/231) | 0 |
| C5 — Error→ProblemDetails + adaptateur AspNetCore | [#232](https://github.com/Reefact/first-class-errors/issues/232) *(inclut les aides hôte HV12)* | 2 |
| C6 — Analyseur d'Outcome ignoré | [#233](https://github.com/Reefact/first-class-errors/issues/233) | 0 |
| HV1 — Hubs de documentation + liens vers les exemples | [#234](https://github.com/Reefact/first-class-errors/issues/234) | 0 |
| HV2 — Error.ToLogModel + schéma de champs | [#235](https://github.com/Reefact/first-class-errors/issues/235) | 2 |
| HV3 — Outcome.Try | en vol : branche `claude/outcome-try-feature-z1u1n4` (avec FCE019–FCE021) | 2 |
| HV4 — Fournisseurs de correctifs de code | [#236](https://github.com/Reefact/first-class-errors/issues/236) | 4 |
| HV5 — fce lint | [#237](https://github.com/Reefact/first-class-errors/issues/237) | 4 |
| HV6 — Durcissement de la confiance des analyseurs | [#238](https://github.com/Reefact/first-class-errors/issues/238) harnais + cohérence · [#239](https://github.com/Reefact/first-class-errors/issues/239) conflit ErrorContextKey | 1 |
| HV7/8 — Politique bilingue + contrôles de synchronisation | [#240](https://github.com/Reefact/first-class-errors/issues/240) | 0 |
| HV9 — Lacunes de la référence des workflows | [#241](https://github.com/Reefact/first-class-errors/issues/241) | 0 |
| HV10 — Symétrie d'API · contrat d'extensibilité | [#242](https://github.com/Reefact/first-class-errors/issues/242) symétrie · [#243](https://github.com/Reefact/first-class-errors/issues/243) extensibilité | 1 |
| HV11 — Durcissement pré-v1 du binder | [#244](https://github.com/Reefact/first-class-errors/issues/244) | 1 |
| HV13 — Analyseurs pour les consommateurs binder seul | [#245](https://github.com/Reefact/first-class-errors/issues/245) | 1 |
| HV14 — Contrôle de version du contrat de documentation | [#246](https://github.com/Reefact/first-class-errors/issues/246) | 4 |
| HV15 — Élagage de fce generate (+ exemples --service-name) | [#247](https://github.com/Reefact/first-class-errors/issues/247) | 0/4 |
| HV16 — Élargir la page de comparaison | [#248](https://github.com/Reefact/first-class-errors/issues/248) | 2 |
| HV17 — Adoption de la reproductibilité (+ ErrorAssertion) | [#249](https://github.com/Reefact/first-class-errors/issues/249) · docs d'accueil JustDummies : existante [#218](https://github.com/Reefact/first-class-errors/issues/218) | 4 |
| HV18 — Statut de l'ADR-0025 | existante [#220](https://github.com/Reefact/first-class-errors/issues/220) (audit JustDummies) | 0 |
| HV19 — Surface de santé communautaire | [#251](https://github.com/Reefact/first-class-errors/issues/251) | 0 |
| HV20 — Réconcilier le registre des releases | [#250](https://github.com/Reefact/first-class-errors/issues/250) | 0/3 |
| HV21 — Durcir code_review.md | [#252](https://github.com/Reefact/first-class-errors/issues/252) | 0 |
| M — ADR lockstep/IVT | [#253](https://github.com/Reefact/first-class-errors/issues/253) | 1 |
| Backlog valeur moyenne/basse | [#254](https://github.com/Reefact/first-class-errors/issues/254) | 4 |
