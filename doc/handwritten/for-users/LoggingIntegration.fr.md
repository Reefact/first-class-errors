# Intégrer FirstClassErrors au logging structuré
...
Cela logge les deux clés les plus importantes en ligne, mais ce n’est pas toute l’erreur. Les propriétés restantes — type, contexte, erreurs internes et champs d’infrastructure — doivent être sérialisées via une seule projection partagée plutôt que réénumérées dans chaque `catch` ; voir [Centraliser la projection vers un modèle de log](#centraliser-la-projection-vers-un-modèle-de-log) plus bas.
...
Les clés de contexte constituent un vocabulaire opérationnel. Gardez leurs noms stables et vérifiez que les valeurs sont sûres pour la destination de logs visée. Un identifiant de paiement loggé sous `PaymentId` doit rester `PaymentId` dans chaque erreur et chaque service — sans dériver vers `Id` ici, `PaymentIdentifier` là, et `payment_id` ailleurs. C’est un nom de clé stable qui permet à une seule requête de corréler le même fait dans tout le système.

Voir [Contexte d’erreur](ErrorContext.fr.md) pour la conception des clés et les règles de sécurité des données.

## Centraliser la projection vers un modèle de log

Ne réénumérez pas les propriétés de l’erreur dans chaque `catch`. Projetez une `Error` vers un modèle de log une seule fois, à un seul endroit, et réutilisez-le partout où un échec est loggé.

La projection doit être récursive : `DiagnosableException` n’utilise pas `Exception.InnerException` pour l’arbre diagnostique FirstClassErrors — les causes et erreurs agrégées vivent dans `error.InnerErrors`. (Une `DiagnosableException` peut néanmoins porter une exception runtime ordinaire — une erreur d’E/S, par exemple — sur `Exception.InnerException` ; ce canal transporte la cause technique avec sa stack trace, pas des erreurs modélisées : loggez-le comme n’importe quelle cause d’exception.) Elle doit aussi ajouter les champs propres à l’infrastructure lorsque l’erreur est une `InfrastructureError` :
...
Chaque `catch` ou handler d’outcome logge alors l’erreur entière en un seul appel :
...
`{@FirstClassError}` est la syntaxe de destructuration de [Serilog](https://github.com/serilog/serilog) : elle sérialise le modèle de log comme donnée structurée au lieu d’appeler `ToString()`. Avec `Microsoft.Extensions.Logging` seul, l’objet est capturé mais rendu par le provider actif ; utilisez un provider qui sérialise l’état (ou `BeginScope`) pour garder les champs requêtables. La projection elle-même ne change pas selon le framework de logging — c’est tout l’intérêt de la centraliser.
...
Le mécanisme principal est une propriété de log structurée telle que `error.documentationUrl`, que n’importe quelle requête ou dashboard peut exposer directement.

`Exception.HelpLink` est une option secondaire et dépendante des outils : seuls certains outils la lisent, alors renseignez-la comme une commodité pour ces outils plutôt que comme la voie de navigation principale.
