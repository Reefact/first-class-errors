...
* Mutation testing gates every pull request on the files it changed, for the five
  shipped libraries, through two independent checks — one for the FirstClassErrors
  libraries, one for the JustDummies packages (decision: ADR-0043). A test that
  *executes* new code without *asserting* it will pass `dotnet test` and still
  fail that gate. Reproduce it on a branch with
  `dotnet tool restore && dotnet stryker --config-file build/stryker/<library>.json --since:$(git merge-base origin/main HEAD)`;
