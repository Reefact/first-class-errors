...
This repository merges pull requests by **rebasing** them onto `main`: every
commit on the branch is replayed into `main`'s history, so cleaning it up before
merge is not cosmetic.
